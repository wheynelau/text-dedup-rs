...
class MetaArgs:
    ...
    def option_group(func):
        ...
        @optgroup.option(
            "--batch_size",
            type=int,
            help="Batch size for deduplication",
            default=10_000,
        )
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
...
class UniSimArgs:
    ...
    def option_group(func):
        ...
        @optgroup.option(
            "--return_embeddings",
            help="Return embeddings instead of scores",
            is_flag=True,
            default=False,
        )
        @optgroup.option(
            "--use_accelerator",
            help="Use accelerator for index creation",
            is_flag=True,
            default=False,
        )
